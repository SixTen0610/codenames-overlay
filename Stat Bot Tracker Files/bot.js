...
const CONFIG = {
  ...
  obsPassword: process.env.OBS_WEBSOCKET_PASSWORD || '',   // from .env
  ...
  discord: {
    ...
    webhookUrl: process.env.DISCORD_WEBHOOK_GAMELINKS || '',  // ← Read Game-Links WebHook from .env
    ...
    announcement: {
      enabled:    true,
      webhookUrl: process.env.DISCORD_WEBHOOK_ANNOUNCE || '',  // ← Read Going Live WebHook from .env
    ...
    stats: {
      enabled:         true,
      webhookUrl: process.env.DISCORD_WEBHOOK_STATS || '',     // ← Read Stats Webhook from .env
  ...
  story: {
    enabled:  true,
    apiKey: process.env.ANTHROPIC_API_KEY || '',    // ← Read Anthropic Key from .env
...
reconnect: {
    ...
    failuresBeforeReconnect: 5,    // consecutive fatal poll errors (any class) before a relaunch fires
    evalTimeoutMs:           8000, // frozen-tab watchdog: max wait for one poll evaluate before it's treated as frozen
  },
...
let pollInFlight      = false;  // guard: a poll evaluate is still running — prevents stacking while a tab is frozen
...
// ── Navigation: domcontentloaded, NOT networkidle2 (mode-2 fix) ──
  // codenames.game holds a live socket open between games, so networkidle2
  // never settles and the old code hit "Navigation timeout of 30000ms" on
  // every rejoin — which made auto-reconnect burn all its attempts and give
  // up. domcontentloaded fires as soon as the DOM is parsed; a short backoff
  // loop covers a genuinely transient navigation failure.
  {
    const navAttempts = 3;
    let navOk = false;
    for (let i = 1; i <= navAttempts; i++) {
      try {
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 20000 });
        navOk = true;
        break;
      } catch (e) {
        const delay = Math.min(8000, 2000 * Math.pow(2, i - 1));
        log(`⚠️  Navigation attempt ${i}/${navAttempts} failed: ${e.message}`
            + (i < navAttempts ? ` — retrying in ${Math.round(delay / 1000)}s` : ''));
        if (i < navAttempts) await new Promise(r => setTimeout(r, delay));
      }
    }
    // Throw on total failure so joinRoom rejects → attemptReconnect backs off
    // and relaunches a fresh browser, rather than polling a half-loaded page.
    if (!navOk) throw new Error(`navigation failed after ${navAttempts} attempts (domcontentloaded)`);
  }
  log('✅ Joined room: ' + url);
...
// ─────────────────────────────────────────────────────────
// FATAL-ERROR CLASSIFIER + POLL WATCHDOG (mode-3 support)
// All three classes mean "the spectator slot is dead/frozen → relaunch":
//   DETACHED    — frame/context/target gone (room emptied between games)
//   NAV_TIMEOUT — navigation never settled
//   FROZEN_TAB  — headless tab froze; Runtime.callFunctionOn times out. This
//                 class used to slip past the old detached-frame check and
//                 spam the log forever without ever triggering a reconnect.
// ─────────────────────────────────────────────────────────
function classifyFatalError(err) {
  const msg = String((err && err.message) || err || '');
  // Check FROZEN_TAB first so a callFunctionOn timeout isn't read as NAV_TIMEOUT.
  if (/Runtime\.callFunctionOn|callFunctionOn timed out/i.test(msg)) return 'FROZEN_TAB';
  if (/Navigation timeout of|net::ERR_/i.test(msg))                  return 'NAV_TIMEOUT';
  if (/detached Frame|Execution context was destroyed|Target closed|Session closed|Protocol error/i.test(msg))
                                                                     return 'DETACHED';
  return null;
}

// Race a promise against a timeout so a hung page.evaluate (frozen tab) REJECTS
// instead of blocking the poll loop forever. The orphaned evaluate keeps
// running until the browser is torn down on reconnect — which is fine, since
// reconnect closes the whole browser anyway.
function raceTimeout(promise, ms, timeoutMsg) {
  let t;
  const timeout = new Promise((_, reject) => {
    t = setTimeout(() => reject(new Error(timeoutMsg)), ms);
  });
...
async function pollGameState() {
  ...
  if (pollInFlight) return;        // previous poll still running (likely a frozen tab) — don't stack
  pollInFlight = true;
  try {
    const evalPromise = page.evaluate((colorMap) => {
      // ── Helper ──
    ...
    // ── Frozen-tab watchdog (mode-3 fix) ──
    // If the headless tab freezes (resource starvation after hours), the
    // evaluate above hangs forever — pollGameState never returns, the failure
    // counter never moves, and reconnect never fires. Racing it against a
    // timeout converts the hang into a FROZEN_TAB rejection the catch can act on.
    const state = await raceTimeout(
      evalPromise,
      (CONFIG.reconnect && CONFIG.reconnect.evalTimeoutMs) || 8000,
      'Runtime.callFunctionOn timed out (frozen-tab watchdog)'
    );
    ...
    // Classify across ALL three failure modes — not just detached frames. The
    // old isDetachedFrame check missed FROZEN_TAB (Runtime.callFunctionOn),
    // which is why the bot used to spam that error forever without reconnecting.
    const cls = classifyFatalError(e);

    if (cls) {
      consecutivePollErrors++;
      const threshold = (CONFIG.reconnect && CONFIG.reconnect.failuresBeforeReconnect) || 5;
      if (consecutivePollErrors === threshold) {
        const why = cls === 'FROZEN_TAB' ? 'headless tab frozen (callFunctionOn timed out)'
                  : cls === 'NAV_TIMEOUT' ? 'navigation stalled'
                  : 'browser frame detached (room likely emptied between games)';
        log(`⚠️  ${why} — ${consecutivePollErrors} consecutive poll failures.`);
        stopPolling();
        ...
        log(`⚠️  Poll error [${cls}]: ${e.message}`);
      }
      // suppress repeated identical errors between the first and the threshold
    } else {
      // Unknown/non-fatal error — log it, but don't count it toward a reconnect.
      log('⚠️  Poll error: ' + (e.message || e));
    }
  } finally {
    pollInFlight = false;   // always release the guard, even on a hung/rejected poll
  }
}
