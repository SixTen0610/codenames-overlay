...
const CONFIG = {
  ...
  story: {
    ...
    model:    'claude-3-5-haiku-20241022', // cheap + fast; good enough for short comedic recaps
...
let pollTimer  = null;
let roomUrl    = null;
let discordPostOnStart = true;  // runtime toggle, controlled via control panel
...
// Pending game-end check — when assassin reveal coincides with full-board flip,
// hold off attributing it for one poll cycle to let instruction text catch up.
let pendingGameEndCheck = null;  // { timestamp, suspectedAssassin: word|null }
...
async function joinRoom(url) {
  ...
  if (discordPostOnStart && !isResumedSession) {
  ...
  try {
    ...
    if (isDetachedFrame) {
      ...
      if (consecutivePollErrors === 5) {
        log('⚠️  Browser frame detached — pausing polling. Click Start again to reconnect.');
        stopPolling();
...
async function processState(state) {
  ...
  if (state.activeTurn && state.activeTurn !== lastBroadcastTurn) {
  ...
  // ── Win detection (deterministic) ──
  // Totals follow from firstTeam: starter has 9, other has 8. Remaining is
  // total minus that color's settled reveals — the exact math verified against
  // a live board (red 9-3=6, blue 8-3=5).
  ...
  // Win text lives in the INSTRUCTION slot ("RED TEAM WINS!"), never `banner`
  // (that's an ad). Guard against a stale banner over a fresh board by
  // requiring at least one real colored reveal to be present.
  ...
  const assassinHit = revealedColors.assassin > 0;
  ...
  if (bannerWinner) {
    ...
    // The guessing team hit the assassin and loses; the other team wins.
    const loser = state.activeTurn || lastBroadcastTurn || 'blue';
    winner = loser === 'blue' ? 'red' : 'blue';
    reason = 'assassin';
  } else if (redTotal !== null && redLeft === 0) {
    winner = 'red';  reason = 'all red cards revealed';
  } else if (blueTotal !== null && blueLeft === 0) {
    winner = 'blue'; reason = 'all blue cards revealed';
  }

  if (winner) {
    if (assassinHit) {
      const loser = winner === 'blue' ? 'red' : 'blue';
      const hit = Object.entries(state.cards).find(([w, d]) => d.color === 'assassin' && d.revealed);
      const hitWord = hit ? hit[0] : '(assassin)';
      log(`💀 ASSASSIN HIT: ${hitWord} — ${loser.toUpperCase()} loses!`);
      await onAssassin(hitWord, loser, state.players);
    }
  ...
  // Only red/blue reveals are knowable to a spectator; logs settled flips.
  Object.entries(state.cards).forEach(([word, data]) => {
...
async function onGameEnd(winner, players, cards) {
  // Stat crediting now happens in the v7 overlay's botCreditGame() function,
  // which has full role history and respects opt-ins. The bot just broadcasts
  // the game-end event and the overlay does the rest.
  log(`🏆 Broadcasting game end → ${winner.toUpperCase()} wins`);
  broadcast({ type: 'gameEnd', winner });

  // Generate a comedic AI recap (fire-and-forget — doesn't block anything)
  if (CONFIG.story && CONFIG.story.enabled) {
    generateGameStory(winner, players, cards || prevState.cards)
      .catch(e => log(`⚠️  Story generation error: ${e.message}`));
  }
}

// ─────────────────────────────────────────────────────────
// AI GAME STORY — comedic recap via Claude API
// ─────────────────────────────────────────────────────────
async function generateGameStory(winner, players, cards) {
  ...
  // Gather the winning team's revealed cards (their color = winner color).
  // At game end the full board is revealed, so we can see them all.
  const winningWords = Object.entries(cards || {})
    .filter(([w, d]) => d.color === winner && d.revealed)
    .map(([w]) => w);

  // Gather winning team's player names from role history
  const winningPlayers = [];
  Object.keys(roleHistory).forEach(name => {
    const combos = [...roleHistory[name]];
    const onWinningTeam = combos.some(c => c.startsWith(winner + ':'));
    if (onWinningTeam) {
      const isSpy = combos.some(c => c === winner + ':SM');
      winningPlayers.push({ name, role: isSpy ? 'spymaster' : 'agent' });
    }
  });

  // Need at least a couple words to make a story
  ...
  const spymasters = winningPlayers.filter(p => p.role === 'spymaster').map(p => p.name);
  const agents     = winningPlayers.filter(p => p.role === 'agent').map(p => p.name);

  // Build the prompt
  const teamLabel = winner === 'blue' ? 'Blue' : 'Red';
  const promptText =
    `You are a comedy writer recapping a game of Codenames for a Twitch stream. ` +
    `The ${teamLabel} team just won. Write a SHORT (2-3 sentences, max 60 words) absurd, ` +
    `comedic spy-thriller recap that weaves in these code words they guessed: ` +
    `${winningWords.join(', ')}. ` +
    (spymasters.length ? `The mastermind spymaster(s): ${spymasters.join(', ')}. ` : '') +
    (agents.length ? `The field agents: ${agents.join(', ')}. ` : '') +
  ...
  try {
    ...
    const entry = { gameNum, winner, story: story.trim(), ts: Date.now() };
    ...
    log(`📖 Game ${gameNum} recap: "${story.trim().substring(0, 80)}..."`);

    // Broadcast to overlay (end-credits reads this)
    broadcast({
      type: 'gameStory',
      gameNum,
      winner,
